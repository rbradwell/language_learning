...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      console.log('Creating fill_blanks exercises...');
      ...
      // Check if sentences exist for each category using JOIN with Categories table
      ...
      if (newExercises.length > 0) {
        await queryInterface.bulkInsert('SentenceCompletionExercises', newExercises, { transaction });
  ...
  async down(queryInterface, Sequelize) {
    ...
    try {
      // Delete exercises for fill_blanks trail steps
      await queryInterface.sequelize.query(`
        DELETE FROM "SentenceCompletionExercises" 
        WHERE "trailStepId" IN (
          SELECT id FROM "TrailSteps" WHERE type = 'fill_blanks'
        )
      `, { transaction });
